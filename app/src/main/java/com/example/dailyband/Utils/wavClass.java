...
public class wavClass {
    ...
    public String tempWavFile = "final_record.wav";
    final int bpp = 16;
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        if (requestCode == PERMISSION_REQUEST_CODE) {
            ...
            if (allPermissionsGranted) {
                // 권한이 허용된 경우 녹음을 시작합니다.
                startRecording();
            } else {
                // 권한이 거부된 경우 Toast 메시지를 표시하거나 다른 처리를 할 수 있습니다.
                Toast.makeText(context, "권한이 거부되었습니다. 녹음을 시작할 수 없습니다.", Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    public void startRecording() {
        try {
            if (ActivityCompat.checkSelfPermission(context, Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED) {
                ...
                Toast.makeText(context, "이상하네....", Toast.LENGTH_SHORT).show();
                return;
            }
    ...
    public void stopRecording(){
        try{
            if(isRecording) {
                ...
                Toast.makeText(context, "아직 녹음하지 않았습니다.", Toast.LENGTH_SHORT).show();
            }
        }
        catch (Exception e){
            e.printStackTrace();
